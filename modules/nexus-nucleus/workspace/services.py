...
def _send_invite_email(company, email: str, redirect_to: str | None) -> tuple[bool, str | None]:
    """
    Email the invitee through Supabase's admin invite API -- only when this
    deployment holds the identity project's service key. Returns
    (email_sent, note): the note says why nothing was sent, so the client
    can tell the inviter to pass the steps on instead.

    The invite also seeds the new account's user metadata with this server
    (nx_servers), which the web app mirrors as the saved-server list -- the
    invitee sees the server on their launcher without typing an address.
    """
