...
from . import chat_services as svc
...
@shared_task(bind=True, queue="neuralops", name="authn.chat_tasks.generate_ai_response")
def generate_ai_response(self, topic_id: str, ai_message_id: str) -> str:
    ...
    # ------------------------------------------------------------------ #
    history = svc.build_history(topic_id, exclude_message_id=ai_message_id, limit=20)
    if not history:
