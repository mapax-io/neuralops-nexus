...
- As a Project Admin, I should still not be able to create, delete, or see the key on an AI Model — `ai_model.create`/`delete` remain Company-scope only (a model's create/delete touches a real provider key). Personas are mine to manage inside my project: `persona.create`/`update`/`delete` are Project-scope since the AI-resource permission redesign.
- As anyone without `persona.mention` in a topic (a Viewer, or someone outside it), my message still posts but no persona answers it, and I am told why — the server refuses the trigger, not just the picker.
