...
| 0.18.9  | 2026-09-14 | **Usage and budgets on the AI models tab.** Each card shows this month's tokens and cost (or "cost unknown" / "(partial)" when the server could not price every call), a bar per budget in ok/warn/stopped tone with what is left, a "90% of budget" or "budget reached" chip, the reset date when stopped, and for an OpenRouter key the provider's own remaining. Register and edit dialogs take a monthly token budget and a monthly dollar budget (blank = none; clearing sends 0). One `GET /model-configs/usage/` feeds the whole tab; a server without the route shows the cards exactly as before. Plan phase 1 (DECISIONS §22). |
